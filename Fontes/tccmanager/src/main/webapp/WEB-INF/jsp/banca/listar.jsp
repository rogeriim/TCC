						...
						<c:forEach items="${bancaList}" var="banca">
							<tr>
								...
								<td class="actions"><c:if
										test="${banca.getStatus() == 'FECHADO' }">
										<a class="btn btn-primary btn-xs"
											href="ver?id=${banca.getId()}">Visualizar</a>
									</c:if> <c:if test="${banca.getStatus() == 'ABERTO' }">
										<a class="btn btn-primary btn-xs"
											href="<c:url value="/solicitacao/criar?id=${banca.getId()}"/>">Criar
											Solicitação</a>
										<a class="btn btn-primary btn-xs" data-toggle="modal"
											id="${banca.getId()}" data-target="#close-modal">Fechar Banca</a>
										<a class="btn btn-primary btn-xs"
											href="editar?id=${banca.getId()}">Editar Informações</a>
										<a class="btn btn-danger btn-xs" data-toggle="modal"
											id="${banca.getId()}" data-target="#delete-modal">Excluir</a>
									</c:if></td>
							</tr>
						</c:forEach>
					</tbody>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">&times;</button>
						<h4 class="modal-title" id="myModalLabel">Fechar Banca</h4>
					</div>
					<div class="modal-body">Se você fechar esta banca, ela
						ficará com status FECHADO e não será possível alterá-la nem
						excluí-la posteriormente. Deseja realmente fechá-la?</div>
					<div class="modal-footer">
						<a type="submit" class="btn btn-primary" id="fechar">Sim</a> <a
							type="button" class="btn btn-default" data-dismiss="modal">Não</a>
					</div>
				</div>
			</div>
